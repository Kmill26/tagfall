...
const CATEGORY_ICONS = { cleaning: 'sparkle', tools: 'hammer', kitchen: 'pan', seasonal: 'leaf', teachers: 'book' };
...
export const icon = (name) => ({
  ...
  book: '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3.5h11.2A2.3 2.3 0 0 1 19.5 5.8V20H8.1A2.1 2.1 0 0 0 6 22.1V3.5Zm1.7 1.6v14.3c.3-.1.7-.2 1.1-.2H17.8V5.8c0-.4-.3-.7-.7-.7H7.7Z"/></svg>',
}[name] || '');
...
export const placeholder = (category, label = '') => `<div class="product-placeholder product-placeholder--${escapeHtml(category)}" aria-label="${escapeHtml(label)} product image placeholder" role="img">${icon(CATEGORY_ICONS[category] || category || 'pan')}</div>`;
